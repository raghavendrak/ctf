...
namespace CTF {
  ...
  void read_sparse_from_file_base(const char * fpath, bool with_vals, bool rev_order, Tensor<dtype> * T){
    ...
    //printf("Able to deallocate datastr[0]\n");
    CTF_int::cdealloc(datastr);
    //printf("Able to deallocate datastr\n");

    printf("writing my_nvals: %lld\n", my_nvals);
    //for (int64_t i = 0; i < my_nvals; i++) printf("pairs[i].k %lld pairs[i].d: %ld\n", pairs[i].k, pairs[i].d);
    T->write(my_nvals,pairs);
    printf("Finished writing T->nnz_tot: %lld\n", T->nnz_tot);

    T->sr->pair_dealloc((char*)pairs);
    printf("T->nnz_tot: %lld\n", T->nnz_tot);
  }
